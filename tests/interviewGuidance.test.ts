...
describe('interview prompt guidance extensions', () => {
  it('injects [참고 법령·판례] header and groundingText output before the slot key spec', () => {
    ...
    const grounding = groundingText(false);
  ...
  it('passes specialEd.isSpecialEd through to groundingText', () => {
    const p = buildInterviewSystemPrompt({ isSpecialEd: true, disabilities: [] });
    const grounding = groundingText(true);
    expect(p).toContain(grounding);
  });
